...
# Fonction pour vérifier l'existence du fichier data.csv
...
afficher_aide() {
    ...
    echo ""
    echo "Exemples:"
    echo "  ./script.sh -D1    # Exécute le traitement D1"
    echo "  ./script.sh -D2    # Exécute le traitement D2"
    echo ""
...
# Puis, vous pouvez appeler cette fonction dans votre script où nécessaire, par exemple :
case "$1" in
    -h)
        afficher_aide
        ;;
...
EOF
    ...
    if [ -e resT.txt ]; then
        mv tempT.txt temp/
        echo "Le resT.txt a été déplacé dans le dossier temp."
    else
