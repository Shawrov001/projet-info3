...
Node *insertByName(Node *node, int key, char nom[], int count) {
    // Étape 1: Insertion standard de l'élément BST par ordre alphabétique du nom
    if (node == NULL){
        return newNode(key, nom, count);
    }
...
int main() {
    ...
    fclose(fichier);
    fichier=fopen("temp2.txt","w");

    // Utilisation de l'arbre AVL ici...
    Node *AVLalpha=NULL;

    int compteur=0;
    printf("Les villes les plus traversés sont :\n\n");
    addPlusGrand(racineAVL, &compteur,&AVLalpha);

/**
    printf("\n\n\n\n\n");
    afficherArbre(AVLalpha,0);
    printf("\n\n");
